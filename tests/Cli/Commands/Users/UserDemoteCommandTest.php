...
use App\Cli\Commands\Users\UserDemoteCommand;
...
{
    private UserDemoteCommand $command;
    ...
    public function testWhenPayloadStatusIsNotUpdated(): void
    {
        ...
        self::assertFalse($userModel->isAdmin);
    }
    ...
    public function testWhenPayloadStatusIsUpdated(): void
    {
        ...
        $output->expects(self::once())
            ->method('writeln')
            ->with(
                self::equalTo('<fg=green>User was demoted from admin</>')
            );
    ...
    private function internalSetup(): void
    {
        ...
        $this->command = new UserDemoteCommand(
            $this->mockQuestionService(),
            $this->mockUserApi()
        );

        self::assertSame('user:demote', $this->command->getName());
    }
